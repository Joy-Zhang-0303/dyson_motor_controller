...
/*===== define the array data (Read Only)=====*/ 
...
void loop()
{
  ...
  int is_1 = analogRead(IS_1);
  int is_2 = analogRead(IS_2);

  if (ENABLE_CURRENT_LIMIT_CHECK && ((is_1 > CURRENT_LIMIT_ANALOG_COUNTER) || (is_2 > CURRENT_LIMIT_ANALOG_COUNTER)))
  {
    cli();
    disable_driver();
  }
